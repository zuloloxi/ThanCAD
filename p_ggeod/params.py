...
from math import pi
from . import ellipsoid, mercator
...
def fromFile(fr):
    ...
    L.append(float(next(fr)))
    icod = int(L[1])
    if icod not in (0, 1, 2, 101, 102, 1001): raise ValueError("Unknown code of geodetic projection: %d (valid codes  are 0, 1, 2, 101, 102, 1001)" % (icod,))
    for i in xrange(2, n):
        L.append(float(next(fr)))
    return L
...
def toProj(L):
    "Return the geodetic projection objects which is defined by the parameters L; may reaise ValueError, IndexError."
    icod = int(L[0])
    ...
    if icod == 0:
        ...
        return mercator.GeodetGRS80(EOID=EOID)
    else:
        raise ValueError("Unknown code of geodetic projection: %d (valid codes  are 0, 1, 2, 101, 102, 1001)" % (icod,))
