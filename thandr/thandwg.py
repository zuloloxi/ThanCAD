...
#        thanEdus
    ...
    def thanElementTag(self, elem, cl=None):
        ...
        if cl == None: cl = self.thanLayerTree.thanCur
...
#        im1 = im1.filter(ImageFilter.SMOOTH)
    ...
    def thanRepairThc(self):
        "Try to create some new objects that were not present in older versions of .thc."
        thanVarsDef(self.thanVar)
        try: self.thanObjects
        except: self.thanObjects = {}
        thanObjsDef(self.thanObjects)
        for lay in self.thanLayerTree.dilay.itervalues():
