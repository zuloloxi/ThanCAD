...
class ThanGrid:
  ...
  Normaly the application need not be a class. Butr in the future, multiple
  ...
  def thanDo(self, proj, akl, dx, dy, ikan, cp):
      ...
      xp = [c[0] for c in cp]
      yp = [c[1] for c in cp]
      m = Mesa(xp[0],yp[0],xp[1],yp[1],xp[2],yp[2],xp[3],yp[3])
...
#-----Find grid points inside the polygon
  ...
  def __del__(self):
        "Inform that grid died for debuggin reasons."
...
class Mesa:
    ...
    def __del__(self):
        "Inform that object died for debugging reasons."
	prg("thaneng.Mesa %s is deleted" % self)
