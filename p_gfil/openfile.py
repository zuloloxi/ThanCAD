...
import sys
from p_ggen import path, Struct, prg, thanUnicode, inpStrB, Canc
import p_gtkuti
import filmed, ffff, opgui, opguitrans, fildat
...
#          if kat[:1] == ".": kat = kat[1:]
          ...
          if frw != None: return frw
          p_gtkuti.thanGudModalMessage(win, "%s: %s" % (fn+kat, DLFILACC), "Error opening file", p_gtkuti.ERROR)
...
import p_gtkuti, p_gtkwid, p_grun
T = p_gtkwid.Twid
...
def runMediate(app, pexpectline=True, popen=False):
    ...
    try:
      ...
      pdir = fn.parent
      fnmed = pdir / "mediate.tmp"
      closeFiles2()
#      p_gtkuti.thanGudModalMessage(out, "wait", "wait")    #for debugging
      try:
