...
class HippoUrban:
    ...
    def build_cache(self, dtm, prt=p_ggen.prg):
        ...
        self.cache.build_cache(dtm, self.hull, prt, self.devdebug)
        self.minmax(dtm, prt)
...
#        print "hoik=", state.hoik
    ...
    def readGrid(self, fr):
        ...
        while True:
            dl = next(it).split()
            if dl[0] == "$": break
            ca = list(map(float, dl))   #works for python2,3
            self.hull.append(ca)
        ...
        self.uxmin, self.uxmax, self.uymin, self.uymax = map(float, next(it).split())  #works for python2,3
